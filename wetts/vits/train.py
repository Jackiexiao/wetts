...
def run(rank, n_gpus, hps):
    ...
    if hps.model.get("use_mel_posterior_encoder", False):
...
def train_and_evaluate(
    ...
    if rank == 0:
        train_loader = tqdm.tqdm(train_loader, desc="Loading train data")
    else:
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    eval_num = getattr(hps.train, "eval_num", 10)
    with torch.no_grad():
        for batch_idx, (
            x,
            x_lengths,
            ...
            # remove else
            x = x[:eval_num]
            x_lengths = x_lengths[:eval_num]
            spec = spec[:eval_num]
            spec_lengths = spec_lengths[:eval_num]
            y = y[:eval_num]
            y_lengths = y_lengths[:eval_num]
            speakers = speakers[:eval_num]
            break
    ...
    image_dict = {
        "gen/mel_{}".format(i): utils.plot_spectrogram_to_numpy(
    ...
    if global_step == 0:
        image_dict.update(
            {
                "gt/mel_{}".format(i): utils.plot_spectrogram_to_numpy(
                    mel[i].cpu().numpy()
                )
                for i in range(eval_num)
            }
        ...
        audio_dict.update(
            {"gt/audio_{}".format(i): y[i, :, : y_lengths[i]] for i in range(eval_num)}
        )
